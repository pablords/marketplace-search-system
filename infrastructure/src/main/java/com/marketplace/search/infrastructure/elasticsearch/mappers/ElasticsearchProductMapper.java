...
public class ElasticsearchProductMapper {
    ...
    public ProductDocument toDocument(Product product) {
        ...
        document.setStatus(mapStatusToDocument(product.getStatus()));
    ...
    private ProductStatusDocument mapStatusToDocument(ProductStatus status) {
        ProductStatusDocument doc = new ProductStatusDocument();
        doc.setIsActive(status.isActive());
        doc.setHasStock(status.hasStock());
    ...
    private String buildSearchableText(Product product) {
        ...
        product.getInfo().getAttributes().forEach(attr -> 
            searchableText.append(attr).append(" "));
        product.getInfo().getTags().forEach(tag -> 
            searchableText.append(tag).append(" "));
    ...
    private String calculatePriceRange(java.math.BigDecimal price) {
        ...
        if (priceValue < 200) return "100-200";
        if (priceValue < 500) return "200-500";
        if (priceValue < 1000) return "500-1000";
